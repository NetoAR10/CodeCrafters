...
exports.generatePaymentReceipt = async (request, response) => {
    try {
        ...
        rows.forEach(pago => {
            doc.fontSize(12).text(pago.Mes, { continued: true, width: 100 })
        ...
        doc.fontSize(12).text('Créditos', { continued: true, width: 100, align: 'center' })
        ...
        response.status(500).send('Error generando recibo de pago');
    }
};